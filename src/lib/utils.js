...
import { useServerProps, gql } from "@shopify/hydrogen";
...
export function CreateCheckout(addressObject) {
  console.log("Dev: " + JSON.stringify(addressObject));

  const variantId = "gid://shopify/ProductVariant/42694550651079";
  ...
  const STOREFRONT_ACCESS_TOKEN = Oxygen?.env?.PUBLIC_STOREFRONT_API_TOKEN;
  const GRAPHQL_URL =
    "https://custom-checkouts.myshopify.com/api/2020-07/graphql.json";
  ...
  const GRAPHQL_BODY = () => {
    return {
      ...
      body: JSON.stringify({ query: CHECKOUT_MUTATION, variables: { input } }),
    };
  };
  ...
  fetch(GRAPHQL_URL, GRAPHQL_BODY())
    ...
    .then((result) => {
      if (!result.data.checkoutCreate.checkout)
        return console.log(result.data.checkoutCreate.checkoutUserErrors);
